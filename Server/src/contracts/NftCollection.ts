...
export class NftCollection {
    ...
    await wallet.contract.sendTransfer({
      ...
      messages: [
        internal({
          value: "0.05",
          to: this.address.toString({ bounceable: false }),
          body: this.createChangeContentBody(params),
        }),
      ],
      sendMode: SendMode.PAY_GAS_SEPARATELY + SendMode.IGNORE_ERRORS,
    });
  ...
  public createChangeContentBody(params: contentParams): Cell {
    ...
    body.storeRef(contentCell);

    // Royalty
    const royaltyBase = 1000;
    const royaltyFactor = Math.floor(this.data.royaltyPercent * royaltyBase);

    const royaltyCell = beginCell();
    royaltyCell.storeUint(royaltyFactor, 16);
    royaltyCell.storeUint(royaltyBase, 16);
    royaltyCell.storeAddress(this.data.royaltyAddress);
